...
Stage6 现在只使用 `PaintedFlowFieldAsset_Stage6` 作为导航流场来源。旧的 CPU 目标点/Dijkstra 生成器已经从运行链路中移除。

运行时流程：

1. Painter 窗口创建、适配、分配、手绘或预设生成 `PaintedFlowFieldAsset_Stage6`。
2. `GPUInstancingManager_Stage6` 在初始化或重建时读取该资产。
3. C# 将资产方向场上传为 `ComputeBuffer<float2>`。
4. `AgentComputeShader_Stage6.compute` 根据 agent 的世界 XZ 坐标采样方向。
...
- `Scripts/MassEngine/PaintedFlowFieldAsset_Stage6.cs`
  - 保存每个格子的方向、速度和权重。
  - 支持手绘流场数据。
  - 支持 `GenerateUniformDirection` 和 `GenerateConvergeToPoint` 两个预设生成函数。

- `Editor/PaintedFlowFieldPainterWindow_Stage6.cs`
  - 提供 painted flow field 资产创建、Fit、Assign、Load、Save。
  - 提供手绘画布。
  - 提供 Uniform Direction 和 Converge To Point 预设按钮。

- `Scripts/GPUInstancingManager_Stage6.cs`
  - 只上传 `PaintedFlowFieldAsset_Stage6`。
  - 将 `flowFieldDirections` 绑定到 simulation kernel。
  - 每帧上传 flow field 相关 uniform 参数。
  - 提供 Play Mode 右键菜单 `Stage6/Rebuild Flow Field`。

- `Scripts/MassEngine/MassSpatialHashGridSettings_Stage6.cs`
  - 提供 Manager 和 flow field preview 共用的模拟范围/grid 参数计算。

- `Shaders/AgentComputeShader_Stage6.compute`
...
- `Enable Flow Field Navigation`
  - 开启或关闭 Stage6 flow field 转向。

- `Painted Flow Field Asset`
  - Stage6 导航使用的手绘/预设流场资产。
...
4. 选中挂有 `GPUInstancingManager_Stage6` 的测试对象。
...
1. 打开 `MassGPUPhysics/Stage6/Painted Flow Field Painter`。
2. 创建或选择 `PaintedFlowFieldAsset_Stage6`。
3. 点击 `Fit To Manager`。
