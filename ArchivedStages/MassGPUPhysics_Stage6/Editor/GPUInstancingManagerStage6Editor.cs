...
[CustomEditor(typeof(GPUInstancingManager_Stage6))]
public sealed class GPUInstancingManagerStage6Editor : Editor
{
    ...
    {
        var manager = target as GPUInstancingManager_Stage6;
    ...
    private static void DrawStage6ConfigTools(GPUInstancingManager_Stage6 manager)
    {
        ...
        using (new EditorGUI.DisabledScope(!hasConfig || scenarioActive))
        {
            ...
            {
                Undo.RecordObject(manager, "Apply Stage6 Config Assets");
                manager.ApplyConfigAssetsToManager();
                EditorUtility.SetDirty(manager);
            }
        }
    ...
    {
        if (manager.attackerTeamConfig == null && manager.defenderTeamConfig == null)
            return;

        EditorGUILayout.HelpBox(
            "Team combat, spawn, render, and flow-field setup is now resolved from the assigned Stage6 Team Config assets. The old manager fields are kept as hidden runtime cache for migration and editor tools.",
            MessageType.None);
    }

    private static void DrawDefenderMovementWarnings(GPUInstancingManager_Stage6 manager)
    {
        if (manager.defenderMovementMode != GPUInstancingManager_Stage6.DefenderMovementMode.UseDefenderFlowField)
            return;
    ...
    private void DrawVatProfileTools(GPUInstancingManager_Stage6 manager)
    {
        EditorGUILayout.Space(8f);
    ...
    private void DrawFlowFieldPreview(GPUInstancingManager_Stage6 manager)
    {
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            if (GUILayout.Button("Open Painter"))
                PaintedFlowFieldPainterWindow_Stage6.ShowWindow();
        }

        GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview = manager.FlowFieldPreview;
    ...
    private static bool HasValidPreviewArrays(GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
    ...
    private void DrawPreviewMap(Rect rect, GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
    ...
    private static bool HasGpuPreviewTexture(GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
    ...
    private static void DrawRuntimeTargetList(GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
    ...
    private static void DrawRuntimeTargetMarkers(Rect mapRect, GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
        if (preview.dynamicTargets == null || preview.dynamicTargets.Length == 0)
            return;
    ...
    private Texture2D GetOrBuildPreviewTexture(GPUInstancingManager_Stage6.FlowFieldPreviewSnapshot preview)
    {
