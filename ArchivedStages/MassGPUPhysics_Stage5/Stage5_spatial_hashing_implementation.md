...
- `GPUInstancingManager_Stage5` 仍是 MonoBehaviour 主入口，但部分纯运行时辅助逻辑已经拆出：
  - `MassSpatialHashGridSettings_Stage5`：计算空间哈希网格分辨率、总格子数、世界尺寸和原点。
  - `MassAgentSpawnUtility_Stage5`：生成初始 `AgentData` 数组。
  - `MassGpuDrawUtility_Stage5`：创建 AppendBuffer、IndirectArguments buffer、LOD PropertyBlock、运行时 billboard mesh，并同步 VAT 材质参数。
- 空间哈希使用密集缓冲：
