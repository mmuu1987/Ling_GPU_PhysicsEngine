...
Stage5 现在只使用 `PaintedFlowFieldAsset_Stage5` 作为导航流场来源。旧的 CPU 目标点/Dijkstra 生成器已经从运行链路中移除。

运行时流程：

1. Painter 窗口创建、适配、分配、手绘或预设生成 `PaintedFlowFieldAsset_Stage5`。
2. `GPUInstancingManager_Stage5` 在初始化或重建时读取该资产。
3. C# 将资产方向场上传为 `ComputeBuffer<float2>`。
4. `AgentComputeShader_Stage5.compute` 根据 agent 的世界 XZ 坐标采样方向。
...
- `Scripts/MassEngine/PaintedFlowFieldAsset_Stage5.cs`
  - 保存每个格子的方向、速度和权重。
  - 支持手绘流场数据。
  - 支持 `GenerateUniformDirection` 和 `GenerateConvergeToPoint` 两个预设生成函数。

- `Editor/PaintedFlowFieldPainterWindow_Stage5.cs`
...
- `Scripts/GPUInstancingManager_Stage5.cs`
  - 只上传 `PaintedFlowFieldAsset_Stage5`。
  - 将 `flowFieldDirections` 绑定到 simulation kernel。
  - 每帧上传 flow field 相关 uniform 参数。
  - 提供 Play Mode 右键菜单 `Stage5/Rebuild Flow Field`。

- `Scripts/MassEngine/MassSpatialHashGridSettings_Stage5.cs`
...
- `Enable Flow Field Navigation`
  - 开启或关闭 Stage5 flow field 转向。

- `Painted Flow Field Asset`
  - Stage5 导航使用的手绘/预设流场资产。
...
4. 选中挂有 `GPUInstancingManager_Stage5` 的测试对象。
...
1. 打开 `MassGPUPhysics/Stage5/Painted Flow Field Painter`。
2. 创建或选择 `PaintedFlowFieldAsset_Stage5`。
3. 点击 `Fit To Manager`。
