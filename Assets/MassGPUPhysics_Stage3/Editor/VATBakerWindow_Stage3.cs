...
namespace MassGPUPhysics.Stage3
{
    /// <summary>
    /// VAT（顶点动画贴图 / Vertex Animation Texture）烘焙工具。
    ///
    /// 【什么是 VAT？】
    /// VAT 是一种把骨骼动画"烘培"到纹理上的技术。
    /// 传统骨骼动画（SkinnedMesh）每帧需要在 CPU/GPU 上做蒙皮计算（骨骼矩阵 × 顶点位置）。
    /// 当有成千上万个角色时，逐个做蒙皮非常昂贵。
    /// VAT 的思路是：提前把所有动画帧的所有顶点位置、法线"烤"到一张 2D 纹理里，
    /// 运行时 GPU 从纹理采样就能拿到任意帧的顶点数据，完全跳过骨骼计算。
    ///
    /// 【VAT 贴图的像素布局】
    /// VAT 贴图的每一行像素对应模型的一个顶点，每一组行对应动画的一帧：
    ///   - 第 0 帧：行 [0, rowsPerFrame-1] 存储所有顶点在这一帧的位置/法线
    ///   - 第 1 帧：行 [rowsPerFrame, 2*rowsPerFrame-1] 同理
    ///   - 依此类推
    /// 对于顶点 #v 在第 f 帧的位置：
    ///   pixelX = v % texWidth
    ///   pixelY = f * rowsPerFrame + v / texWidth
    ///
    /// 【低模 LOD VAT 的生成方式：顶点聚类减面】
    /// 不是直接删除三角形（会产生破洞），而是把空间上相近的顶点合并成"代表顶点"，
    /// 在新的简化网格上生成新的 VAT 贴图。类比：把高分辨率的顶点云降采样。
    ///
    /// 【使用方式】
    /// 菜单栏 MassGPUPhysics > Stage3 > VAT Baker 打开此窗口，
    /// 拖入带 Animator + SkinnedMeshRenderer 的预制体，选择动画片段，点击 Bake。
    /// </summary>
    public class VATBakerWindow_Stage3 : EditorWindow
    {
        ...
        /// <summary>注册编辑器菜单项：MassGPUPhysics > Stage3 > VAT Baker</summary>
        [MenuItem("MassGPUPhysics/Stage3/VAT Baker")]
        public static void ShowWindow()
        {
            GetWindow<VATBakerWindow_Stage3>("VAT Baker Stage3");
        }
        ...
        /// <summary>
        /// 验证用户输入：检查是否有目标对象、动画片段、以及至少有一个可用的网格渲染器。
        /// </summary>
        ...
        /// <summary>
        /// 核心烘焙流程：
        /// 1. 实例化目标 GameObject（避免修改原始资产）
        /// 2. 逐帧用 AnimationClip.SampleAnimation 采样动画姿态
        /// 3. 用 SkinnedMeshRenderer.BakeMesh 获取蒙皮后的顶点位置/法线
        /// 4. 将顶点数据写入 VAT 贴图（RGBAHalf 格式，每个像素存一个顶点的一帧数据）
        /// 5. 创建合并后的静态 Mesh（所有子网格合并到一个 Mesh 中）
        /// 6. 可选：用顶点聚类生成低模 LOD VAT
        /// 7. 保存为 .asset 文件
        /// </summary>
        private void Bake()
        {
            // 实例化目标对象，避免修改原始 Prefab 资产
            ...
            // 获取实例中所有的 SkinnedMeshRenderer
            SkinnedMeshRenderer[] smrs = instObj.GetComponentsInChildren<SkinnedMeshRenderer>(true);

            // 记录 extraRenderers（非蒙皮附件）在原始 hierarchy 中的路径，然后在实例中查找对应副本
            // 因为 Instantiate 会复制整个层级结构，路径结构保持一致
            ...
            // ── 计算 VAT 贴图尺寸 ──
            // texWidth：每行的像素数 = min(总顶点数, 4096)，4096 是大多数 GPU 支持的最大纹理宽度
            // rowsPerFrame：每帧占用多少行 = ceil(总顶点数 / texWidth)
            // texHeight：总贴图高度 = rowsPerFrame × 帧数
            // 例如：10000 顶点, texWidth=4096 → rowsPerFrame=3, 30 帧 → texHeight=90
            ...
            // ── 创建 VAT 贴图 ──
            // RGBAHalf：每个通道 16 位浮点（半精度），足够存世界坐标位置且比 RGBAFloat 省显存
            // Point 过滤：VAT 是精确数据采样，不能用双线性插值（会混合相邻顶点造成错误变形）
            // Clamp 寻址：防止 UV 越界时重复边缘数据
            ...
            // 在 CPU 端构建像素颜色数组，最后一次性上传到 GPU（避免逐像素 SetPixel 的巨额开销）
