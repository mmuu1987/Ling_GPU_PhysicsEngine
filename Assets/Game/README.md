...
- `F1` 聚焦攻方、`F2` 聚焦守方、`F3` 框住全战场、`F` 聚焦当前选择阵营

移动和撤退命令会在地面显示带阵营颜色的目标十字。镜头滚轮采用有界距离缩放，
飞行/平移也有单帧位移和世界坐标保护；异常输入不会再把 Transform 推到 NaN/Infinity
或极远坐标。右键飞行期间 A/M/H/R 不会误触发军团命令。

> 后续镜头事项：当前 F/F1/F2 聚焦的是配置中的出生阵型/命令范围，不会随 GPU 上的
> 存活群体实时移动。后续需要增加低成本的分队质心遥测，再实现平滑跟随镜头；本阶段暂不扩展。
